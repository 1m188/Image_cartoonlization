...
**选择：** 9 个 `.cs` 文件在 `cs/ImageCartoonlization/` 下：

| 文件 | 职责 |
|------|------|
| `Program.cs` | CLI 入口、`-v` 详细输出、完整流程编排 |
| `CliParser.cs` | 命令行参数手动解析（AOT 兼容）、参数校验、`-h`/`--help` 帮助 |
...
**理由：** 镜像 Rust 的模块结构（一个关切一个文件），已验证简洁可用。每个文件 40-320 行，可独立进行单元测试。`CliParser` 从 `Program.cs` 中拆分以支持独立测试（`CliTests.cs` 含 11 个测试用例）。

### 3. 并行策略：`Parallel.For` 行级并行

**选择：** `System.Threading.Tasks.Parallel.For(0, workers, wk => { /* 处理第 wk 批行 */ })`。
...
**性能优化（第二轮审查）：** 双边滤波内层循环改为直接访问 `Data` 数组（绕过 `GetPixel`/`SetPixel`），每个邻域像素仅读取一次。空间域高斯核从 `float[][]` 锯齿数组改为 `float[]` 一维数组 + 手动索引（`kernel[(rowOff+w)*size + (colOff+w)]`），减少指针跳转并改善缓存局部性。
...
| 手动 CLI 解析容易出错 | 覆盖所有参数组合的测试；参数校验逻辑与 Go/Rust 严格对齐。已加固：-i 后跟标志时报错、数值参数缺值时明确报错 |
| AOT 编译后缺乏动态调试能力 | 开发阶段使用普通 JIT 模式进行 TDD；AOT 仅用于最终发布构建 |
| sigma_d / sigma_r = 0 导致 NaN | BilateralFilter.Apply 入口增加 guard，sigma <= 0 时直接返回原图 |
| ImageData 无效 dimensions 未校验 | 两个构造器均增加 Width/Height/Channels `ArgumentOutOfRangeException` 校验 |
| LabColor API 接收错误通道数 | RgbToLab/LabToRgb/RgbToGray 入口增加 channels != 3 `ArgumentException` 校验 |
| EdgeDetection 尺寸不匹配 | OverlayEdges 入口增加 blurred 与 edgeMask 尺寸一致性校验 |
| GetPixel/SetPixel 越界无提示 | 增加 `(uint)idx >= limit` 边界检查并抛出 `ArgumentOutOfRangeException`（含坐标详情） |
| SaveImage 接受 channels > 3 图像但代码硬编码步长为 3 | 守卫条件从 `img.Channels < 3` 改为 `img.Channels != 3`（第三轮 MPLR L1） |
| CliParser 静默忽略未知参数 | 增加 `knownFlags` 集合校验，未知标志输出错误信息（第三轮 MPLR M1） |
| CliParser 偶发多个 -i 导致行为异常 | 检测多个 -i 出现时明确报错（第三轮 MPLR L2） |
| FloatToByte 对 NaN 输入返回 0 静默截断 | 增加 `float.IsNaN(v)` 显式检查（第三轮 MPLR L3） |
| width * height * channels 整数溢出 | 使用 `(long)` 乘法检测是否超过 `Array.MaxLength`（第三轮 MPLR R1） |
| sigma NaN/Infinity 未经 guard | BilateralFilter.Apply 增加 `float.IsNaN` / `float.IsInfinity` 检查（第三轮 MPLR R4） |
| LabColor / Saturation / EdgeDetection 重复 GetPixel/SetPixel 调用 | 全模块改为直接 Data 数组访问（第三轮 MPLR P1/P2） |
